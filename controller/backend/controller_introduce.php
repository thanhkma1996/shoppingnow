		...
		public function __construct(){
			...
			switch($act){
				case "delete":
					...
					//-------------
					//thuc thi truy van
					$this->model->execute("delete from tbl_introduce where pk_introduce_id=$id");
			...
			$total_record = $this->model->num_rows("select * from tbl_introduce");
			//tinh so trang
			...
			$arr = $this->model->fetch("select * from tbl_introduce order by pk_introduce_id desc limit $from,$record_per_page");
			//load view
			include "view/backend/view_introduce.php";
			//-------------
		}
	}
